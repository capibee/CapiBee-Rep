...
export default function App() {
  ...
  const [user, setUser] = useState<{ id?: string; email: string; roleId: string; roleName?: string; fullName?: string; avatar?: string } | null>(null);
  ...
  const handleLogin = (loggedInUser: { id?: string; email: string; roleId: string; roleName?: string; fullName?: string; avatar?: string } | null) => {
    setIsAuthenticated(true);
  ...
  const userPermissions = useMemo(() => {
    const fullPermissions: Record<string, ModulePermission> = {
      registro_negocios: { active: true, view: true, create: true, edit: true, delete: true },
      clientes: { active: true, view: true, create: true, edit: true, delete: true },
      contabilidad: { active: true, view: true, create: true, edit: true, delete: true },
      mis_negocios: { active: true, view: true, create: true, edit: true, delete: true },
      agentes: { active: true, view: true, create: true, edit: true, delete: true },
      ganancias: { active: true, view: true, create: true, edit: true, delete: true },
      asuntos: { active: true, view: true, create: true, edit: true, delete: true },
      propuestas: { active: true, view: true, create: true, edit: true, delete: true },
      usuarios_roles: { active: true, view: true, create: true, edit: true, delete: true },
      finanzas: { active: true, view: true, create: true, edit: true, delete: true },
      solicitudes: { active: true, view: true, create: true, edit: true, delete: true },
      supabase: { active: true, view: true, create: true, edit: true, delete: true },
    };
  ...
  const renderContent = () => {
    if (isModuleChanging) {
      ...
      const name = pendingModule ? (moduleNames[pendingModule] || 'Panel') : 'Panel de Control';
      return <ModuleLoader moduleName={name} />;
    }

    if (activeModule === 'panel') {
      return <div className="text-white p-8">Panel Principal (Próximamente)</div>;
    }

    if (activeModule === 'registro_negocios' && userPermissions['registro_negocios']?.active) {
      return <Dashboard onLogout={handleLogout} onBack={() => handleModuleChange(null)} />;
    }

    if (activeModule === 'contabilidad' && userPermissions['contabilidad']?.active) {
      return <Contabilidad onLogout={handleLogout} onBack={() => handleModuleChange(null)} />;
    }

    if (activeModule === 'clientes' && userPermissions['clientes']?.active) {
      return <Clientes onLogout={handleLogout} onBack={() => handleModuleChange(null)} />;
    }

    if (activeModule === 'mis_negocios' && userPermissions['mis_negocios']?.active) {
      return <MyBusinesses onLogout={handleLogout} onBack={() => handleModuleChange(null)} />;
    }

    if (activeModule === 'agentes' && userPermissions['agentes']?.active) {
      return <Agentes onLogout={handleLogout} onBack={() => handleModuleChange(null)} />;
    }
    ...
    if (activeModule === 'usuarios_roles' && userPermissions['usuarios_roles']?.active) {
    ...
    if (activeModule === 'finanzas' && (user?.roleId === 'ADMIN_MAESTRO' || userPermissions['finanzas']?.active)) {
      return <Finanzas />;
    }
    ...
    return <ModulesSelection onSelectModule={handleModuleChange} onLogout={handleLogout} userPermissions={userPermissions} />;
  };
