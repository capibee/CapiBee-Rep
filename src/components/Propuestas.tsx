    ...
    <div className="h-full bg-slate-950 p-6 flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-center">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-yellow-400 hover:bg-yellow-500 text-slate-950 px-5 py-3 rounded-xl flex items-center gap-2 font-black transition-all shadow-lg shadow-yellow-500/20 whitespace-nowrap text-sm">
              <Plus size={16} /> Subir Propuesta
           </button>
        </div>
      </div>
          ...
          <table className="w-full text-left min-w-[800px]">
            <thead className="text-slate-500 text-[10px] uppercase tracking-widest sticky top-0 bg-slate-950 z-10">
                <tr>
                    ...
                    <th className="p-4 font-bold hidden">Fecha</th>
            ...
            <tbody className="divide-y divide-slate-800/30">
                {isTableLoading ? (
                    ...
                    currentItems.map((p, index) => {
                        ...
                        return (
                        <tr key={`${p.id}-${index}`} className="hover:bg-slate-900/20 transition-colors">
                            ...
                            <td className="p-4 text-sm text-slate-500 hidden">{new Date(p.createdAt).toLocaleDateString()}</td>
